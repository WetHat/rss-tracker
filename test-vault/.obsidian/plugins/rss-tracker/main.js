...
var _FeedManager = class {
  ...
  */
  ...
  async createFeed(feed, location) {
    var _a2, _b, _c;
    const { title, site, description } = feed, defaultImage = "defaultFeedImage.svg", image = feed.image;
    const dataMap = {
      "{{feedUrl}}": feed.source,
    ...
    if (dashboard && cfg) {
      if (!image) {
        ...
        await this._app.vault.create(imagePath, _FeedManager.RSS_IMAGE_SVG);
      }
      let status;
      try {

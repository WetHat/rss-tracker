...
\u{1F310} Read article [online]({{link}}). \u2934 For other items in this feed see {{feedLink}}.
...
var RSSTrackerSettings = class {
  ...
  // pending value of the RSS default image path
  get rssHome() {
  ...
  async commit() {
    ...
    if (this._rssFeedFolderName && this._rssFeedFolderName !== this._data.rssFeedFolderName) {
      ...
      this._rssFeedFolderName = void 0;
    }
    if (this._rssCollectionsFolderName && this._rssCollectionsFolderName !== this._data.rssCollectionsFolderName) {
      ...
      this._rssCollectionsFolderName = void 0;
    }
    if (this._rssTopicsFolderName && this._rssTopicsFolderName !== this._data.rssTopicsFolderName) {
      ...
      this._rssTopicsFolderName = void 0;
    }
    ...
    if (this._rssDashboardPlacement && this._rssDashboardPlacement !== this._data.rssDashboardPlacement) {
      ...
      this._rssDashboardPlacement = void 0;
    }
    if (this._rssDefaultImagePath && this._rssDefaultImagePath !== this._data.rssDefaultImagePath) {
      ...
      this._rssDefaultImagePath = void 0;
    }
    await this.saveData();
  }
  async loadData() {
...
var HTMLxlate = class _HTMLxlate {
  ...
  async articleAsMarkdown(html, baseUrl) {
    const article = await extractFromHtml(html, baseUrl);
    if (!article) {
      return null;
    }
...
var _RSSitemAdapter = class _RSSitemAdapter extends RSSAdapter {
  ...
  get id() {
    var _a2;
    return (_a2 = this.frontmatter.id) != null ? _a2 : this.link;
  }
  set tags(value) {
    const tagmgr = this.plugin.tagmgr;
    this.frontmatter.tags = value.map((t) => tagmgr.mapHashtag(t.startsWith("#") ? t : "#" + t).slice(1));
  }
  async completeReadingTask() {
    var _a2, _b;
    const app = this.plugin.app, vault = app.vault, tasks = (_b = (_a2 = app.metadataCache.getFileCache(this.file)) == null ? void 0 : _a2.listItems) == null ? void 0 : _b.filter((li) => li.task === " "), first = tasks == null ? void 0 : tasks.first();
    if (first) {
...
var RSScollectionAdapter = class _RSScollectionAdapter extends RSSAdapter {
  static async create(plugin) {
    const folder = await plugin.filemgr.ensureFolderExists(plugin.settings.rssCollectionsFolderPath), file = await plugin.filemgr.createUniqueFile(folder, "New Feed Collection", "RSS Collection"), frontmatter = {
      role: "rsscollection",
      tags: ["nil"],
      allof: [],
      noneof: []
...
var RSSdashboardAdapter = class _RSSdashboardAdapter extends RSSAdapter {
  static getDashboardName(folder, template) {
    return folder.name;
  }
  static getDashboardFolderName(dashboard, template) {
    return dashboard.name;
  }
  static getDashboardPlacementFolder(folder, placement) {
    var _a2;
    return placement === "insideFolder" ? folder : (_a2 = folder.parent) != null ? _a2 : folder.vault.getRoot();
  }
  static getDashboardPath(folder, placement) {
    const dashboardName = _RSSdashboardAdapter.getDashboardName(folder), dashboardPlacementFolder = _RSSdashboardAdapter.getDashboardPlacementFolder(folder, placement);
    return dashboardPlacementFolder.path + "/" + dashboardName + ".md";
  }
  static getDashboard(folder, placement) {
    return folder.vault.getFileByPath(_RSSdashboardAdapter.getDashboardPath(folder, placement));
  }
  /**
  ...
  static createFromFolder(cTor, plugin, folder, placement) {
    const dashboard = _RSSdashboardAdapter.getDashboard(folder, placement);
  ...
  async rename(newFolderName) {
    var _a2, _b, _c, _d;
    const newDashboardName = _RSSdashboardAdapter.getDashboardName(this.folder, newFolderName), newFolderPath = ((_b = (_a2 = this.folder.parent) == null ? void 0 : _a2.path) != null ? _b : "") + "/" + newFolderName, newDashboardPath = ((_d = (_c = this.file.parent) == null ? void 0 : _c.path) != null ? _d : "") + "/" + newDashboardName + ".md";
...
var _RSSfeedAdapter = class _RSSfeedAdapter extends RSSdashboardAdapter {
  static async create(plugin, feed) {
    const { title, site, description } = feed, defaultImage = await plugin.settings.rssDefaultImagePath, image = feed.image, frontmatter = {
      role: "rssfeed",
      aliases: [],
  ...
  get status() {
    var _a2;
    return (_a2 = this.frontmatter.status) != null ? _a2 : '"?"';
  }
  set status(value) {
  ...
  get interval() {
    var _a2;
    return parseInt((_a2 = this.frontmatter.interval) != null ? _a2 : 1);
  }
  /**
  ...
  get itemlimit() {
    var _a2;
    return parseInt((_a2 = this.frontmatter.itemlimit) != null ? _a2 : 100);
  }
  set itemlimit(value) {
  ...
  async update(feed) {
    ...
    const newRSSitems = feed.items.slice(0, this.itemlimit).filter((itm) => !oldItemsMap.has(itm.id));
...
var RSSFeedsDashboardAdapter = class _RSSFeedsDashboardAdapter extends RSSdashboardAdapter {
   ...
   */
  static async create(plugin) {
    const filemgr = plugin.filemgr, settings = plugin.settings, folder = await filemgr.ensureFolderExists(settings.rssFeedFolderPath), placement = settings.rssDashboardPlacement, frontmatter = {
      role: "rssfeed-dashboard"
    }, dashboardName = RSSdashboardAdapter.getDashboardName(folder), dashboardplacement = RSSdashboardAdapter.getDashboardPlacementFolder(folder, placement), dashboard = await filemgr.upsertFile(dashboardplacement, dashboardName, "RSS Feed Dashboard", {}, false);
...
var RenameRSSFeedModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian3.Setting(contentEl).setHeading().setName(`Rename RSS Feed: ${this.feed.file.basename}`);
    new import_obsidian3.Setting(contentEl).setDesc("Enter a new, unique name for the RSS feed.").addText((text) => {
...
var InputUrlModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    const { contentEl } = this;
    new import_obsidian3.Setting(contentEl).setHeading().setName("Feed Url");
    new import_obsidian3.Setting(contentEl).setDesc("Enter the url of the RSS feed:").addText((text) => {
      text.inputEl.addEventListener("keyup", async (evt) => {
        var _a2;
...
var DownloadRSSitemArticleCommand = class extends RSSTrackerCommandBase {
  ...
  checkCallback(checking) {
    ...
    if (active) {
      ...
      feedmgr.downloadArticle(active).then((v) => new import_obsidian4.Notice(`Article content of "${active.basename}" downloaded`));
      return true;
    }
    return false;
  }
  ...
  callback() {
    const modal = new InputUrlModal(this.app, async (result) => {
      ...
      try {
        ...
        new import_obsidian4.Notice(err.message);
      }
    });
    modal.open();
  }
...
var FeedManager = class {
  ...
  */
  ...
  async updateFeeds(feeds, force) {
    ...
    const notice = new import_obsidian5.Notice(`0/${feeds.length} feeds updated`, 0);
...
var RSSTrackerMenuItem = class {
  ...
  get fileMenuHandler() {
    return this.app.workspace.on("file-menu", (menu, file) => {
      this.addItem(menu, file instanceof import_obsidian6.TFile ? file : null);
    });
  }
};
    ...
    if (rssitem && feedmgr.canDownloadArticle(rssitem)) {
      menu.addItem((item) => {
        item.setTitle("Download RSS Item article").setIcon("download").onClick(async () => {
          ...
          new import_obsidian6.Notice(`Article content of "${rssitem.basename}" downloaded`);
        });
      });
    }
  }
...
var RenameRSSfeedMenuItem = class extends RSSTrackerMenuItem {
  constructor(app, plugin) {
    super(app, plugin);
  }
  /**
...
var _DataViewJSTools = class _DataViewJSTools {
  ...
  fromTags(page) {
    var _a2, _b, _c, _d, _e, _f;
    const anyTags = (_b = (_a2 = page.file.etags) == null ? void 0 : _a2.map((t) => _DataViewJSTools.toHashtag(t))) != null ? _b : [], allTags = (_d = (_c = page.allof) == null ? void 0 : _c.map((t) => _DataViewJSTools.toHashtag(t))) != null ? _d : [], noneTags = (_f = (_e = page.noneof) == null ? void 0 : _e.map((t) => _DataViewJSTools.toHashtag(t))) != null ? _f : [];
    let from = [
  ...
  async rssItemHeader(item) {
    ...
    if (opts.showTags) {
      const tags = _DataViewJSTools.getHashtagsAsString(item);
      if (tags) {
        this.dv.span(tags);
      }
    }
  ...
  */
   ...
   */
  async expandableTable(header, rows, label = "Files", expand = void 0) {
    if (expand === void 0) {
      ...
      if (details.open) {
        await this.renderTable(details);
      } else {
        details.addEventListener("toggle", async (evt) => await this.renderTable(evt.target));
      }
    }
  }
  /**
   ...
   *                       `false` to collapse the table by default and render the table on-demand.
   */
  async rssCollectionTable(collections, expand = void 0) {
    const rows = collections.sort((c) => c.file.name, "asc").map((c) => [c.file.link, c.headline]);
    await this.expandableTable(_DataViewJSTools.TABLE_HEADER.rssCollection, rows, "Collections", expand);
  }
  /**
   ...
   *                       `false` to collapse the table by default and render the table on-demand.
   */
  async rssTopicTable(topics, expand = void 0) {
    const rows = topics.sort((t) => t.file.name, "asc").map((t) => [t.file.link, t.headline]);
    await this.expandableTable(_DataViewJSTools.TABLE_HEADER.rssTopic, rows, "Topics", expand);
  }
  /**
   ...
   *                       `false` to collapse the table by default and render the table on-demand.
   */
  async rssFeedTable(feeds, expand = void 0) {
    const rows = feeds.sort((t) => t.file.name, "asc").map((f) => [
      f.file.link,
      f.status,
      _DataViewJSTools.getHashtagsAsString(f),
      f.updated
    ]);
    await this.expandableTable(_DataViewJSTools.TABLE_HEADER.rssFeed, rows, "Feeds", expand);
  }
  /**
   ...
   *                       `false` to collapse the table by default and render the table on-demand.
   */
  async rssFeedDashboard(feeds, expand = void 0) {
    ...
    const rows = feeds.sort((f) => f.file.name, "asc").map((f) => {
      var _a2;
      return [
        f.file.link,
        f.status,
        _DataViewJSTools.getHashtagsAsString(f),
        (_a2 = feedsToCollections.get(f.file.path)) == null ? void 0 : _a2.map((c) => c.file.link).join(" "),
        f.updated
      ];
    });
    await this.expandableTable(_DataViewJSTools.TABLE_HEADER.rssFeedDashboard, rows, "Feeds", expand);
  }
  /**
  ...
  async rssItemTable(items, expand = void 0, label = "Items") {
    ...
    await this.expandableTable(_DataViewJSTools.TABLE_HEADER.rssItem, rows, label, expand);
  }
  /**
   ...
   *                       `false` to collapse the table by default and render the table on-demand.
   */
  async rssItemTableByFeed(items, expand = void 0) {
    if (items.length === 0) {
      this.dv.paragraph("\u26D4");
      return;
    }
  ...
  async rssTaskList(tasks, expand, header = "Items") {
    if (tasks.length > 0) {
      ...
      if (expand) {
        await this.renderTaskList(details);
      } else {
        details.addEventListener("toggle", async (evt) => this.renderTaskList(evt.target));
      }
    } else {
      this.dv.paragraph("\u26D4");
    }
...
var RSSHomeSetting = class extends RSSTrackerSettingBase {
  constructor(settingsTab) {
    super(settingsTab);
    this.setName("RSS feed home folder name").setDesc("The name of the RSS folder containing all RSS related folders, feeds, dashboards and assets.").addText((ta) => {
...
var RSSFeedFolderSetting = class extends RSSTrackerSettingBase {
  constructor(settingsTab) {
    super(settingsTab);
    this.setName("RSS feed folder name").setDesc("The name of the sub-folder in RSS Home containing all subscribed RSS feeds.").addText((ta) => {
      ta.setPlaceholder(DEFAULT_SETTINGS.rssFeedFolderName).onChange((value) => {
      ...
      btn.setIcon("layout-dashboard").setTooltip("Reset the RSS feed dashboard to default").onClick(async (evt) => await RSSFeedsDashboardAdapter.create(this.plugin));
    });
  }
};
var RSSCollectionsFolderSetting = class extends RSSTrackerSettingBase {
  constructor(settingsTab) {
    super(settingsTab);
    this.setName("RSS feed collections location").setDesc("The folder containing all RSS feed collections.").addText((ta) => {
      ta.setPlaceholder(DEFAULT_SETTINGS.rssCollectionsFolderName).onChange((value) => {
      ...
      btn.setIcon("reset").setTooltip("Reset the RSS feed collections location to default").onClick((evt) => {
...
var RSSDefaultItemLimitSetting = class extends RSSTrackerSettingBase {
  constructor(settingsTab) {
    super(settingsTab);
    this.setName("Default RSS item limit").setDesc("The maximum items per feed.").addText((ta) => {
      ta.setPlaceholder(DEFAULT_SETTINGS.defaultItemLimit.toString()).onChange((value) => {
      ...
      btn.setIcon("reset").setTooltip("Reset the RSS item limit default").onClick((evt) => {
        this.settings.defaultItemLimit = DEFAULT_SETTINGS.defaultItemLimit;
      });
    });
  }
};
var RSSTrackerSettingTab = class extends import_obsidian8.PluginSettingTab {
...
var _RSSfileManager = class _RSSfileManager {
  ...
  getDashboardFolder(dashboard, placement) {
    var _a2, _b;
    const folderName = RSSdashboardAdapter.getDashboardFolderName(dashboard), dashboardParent = dashboard.parent;
   ...
   */
  async createUniqueFolder(parentFolder, folderName) {
...
var RSSTagManager = class {
  ...
  async commit(context) {
    if (this._pendingMappings.length > 0) {
      await this._tagmapMutex.lock();
      const file = await this.ensureTagmapExists(), taglist = this._pendingMappings.map((row) => `- ${row.split("|")[1]}`).join("\n");
      if (context) {
...
var RSSTrackerPlugin = class extends import_obsidian10.Plugin {
  ...
  async onload() {
    ...
    this.registerObsidianProtocolHandler("newRssFeed", async (params) => {
      ...
      if (xmlFile) {
        ...
        new import_obsidian10.Notice(`New RSS Feed "${dashboard.file.basename}" created`);
      }
    });
  ...
  async install() {
    ...
    const home = await this.filemgr.ensureRSShomeFolderExists(), placement = this._settings.rssDashboardPlacement;
    ...
    if (!this.settings.rssDefaultImagePath || !this.app.vault.adapter.exists(this.settings.rssDefaultImagePath, true)) {
      const imagePath = await this.app.fileManager.getAvailablePathForAttachment("RSSdefaultImage.svg", this.settings.rssTagmapPath), img = await this.app.vault.create(imagePath, this.settings.rssDefaultImage);

...
var _RSSTrackerSettings = class {
  ...
  get rssFeedFolder() {
    var _a2;
    return (_a2 = this._data.rssFeedFolder) != null ? _a2 : DEFAULT_SETTINGS.rssFeedFolder;
  }
  set rssFeedFolder(value) {
    this._rssFeedFolder = value;
  }
  get rssCollectionsFolder() {
    var _a2;
    return (_a2 = this._data.rssCollectionsFolder) != null ? _a2 : DEFAULT_SETTINGS.rssCollectionsFolder;
  }
  set rssCollectionsFolder(value) {
    this._rssCollectionsFolder = value;
  }
  get rssTopicsFolder() {
    var _a2;
    return (_a2 = this._data.rssTopicsFolder) != null ? _a2 : DEFAULT_SETTINGS.rssTopicsFolder;
  }
  set rssTopicsFolder(value) {
    this._rssCollectionsFolder = value;
  }
...
var FeedManager = class {
  ...
  async updateFeedItems(feedConfig, feed) {
    ...
    for (const itemFile of itemFolder.children.filter((fof) => fof instanceof import_obsidian2.TFile).map((f) => f)) {
    ...
    for (const newItem of newRSSitems) {
      try {
        ...
        new import_obsidian2.Notice(`Could not save '${newItem.fileName}' in feed '${feedConfig.source.name}': ${err.message}`);
      }
    }
  ...
  */
  ...
  async markFeedItemsRead(feed) {
    var _a2, _b;
    const itemFolder = this._app.vault.getFolderByPath(this.getItemFolderPath(feed));
    if (itemFolder) {
      ...
      for (let item of items) {
        ...
        if (first) {
          const data = await this._app.vault.read(item), s = first.position.start.offset, e = first.position.end.offset, newdata = data.substring(0, s) + "- [x]" + data.substring(s + 5);
  ...
  async updateAllRSSfeeds(force) {
    ...
    new import_obsidian2.Notice(`${n}/${promises.length} RSS feeds successfully updated`, 3e4);
  }
  ...
  async downloadArticle(item) {
    ...
    if (link) {
      const itemHTML = await (0, import_obsidian2.request)({
        url: link,
        method: "GET"
    ...
    if (rssitem && feedmgr.canDownloadArticle(rssitem)) {
      menu.addItem((item) => {
        item.setTitle("Download RSS Item article").setIcon("download").onClick(async () => {
          ...
          new import_obsidian4.Notice(`Article content of "${rssitem.basename}" downloaded`);
        });
      });
    }
  }
      ...
      if (feedconfig) {
        menu.addItem((item) => {
          item.setTitle("Update RSS feed").setIcon("rss").onClick(async () => {
            ...
            new import_obsidian4.Notice(`${(_a2 = file == null ? void 0 : file.name) != null ? _a2 : "???"} updated`);
          });
        });
      }
    }
...
var DataViewJSTools = class {
  ...
  get fromFeedsFolderFiles() {
    ...
    if (feedsFolder) {
      return feedsFolder.children.filter((fof) => fof instanceof import_obsidian5.TFile).map((f) => '"' + f.path + '"').join(" OR ");
    } else {
...
var RSSTrackerSettingBase = class extends import_obsidian6.Setting {
...
var RSSCollectionsFolderSetting = class extends RSSTrackerSettingBase {
  constructor(settingsTab) {
    ...
    this.setName("RSS feed collections location").setDesc("The folder containing all RSS feed collections.").addText((ta) => {
      ta.setPlaceholder(DEFAULT_SETTINGS.rssCollectionsFolder).onChange((value) => {
...
var RSSTopicsFolderSetting = class extends RSSTrackerSettingBase {
  constructor(settingsTab) {
    ...
    this.setName("RSS topics location").setDesc("The folder containing all RSS topics.").addText((ta) => {
      ta.setPlaceholder(DEFAULT_SETTINGS.rssTopicsFolder).onChange((value) => {
...
var RSSTrackerSettingTab = class extends import_obsidian6.PluginSettingTab {
...
var _RSSfileManager = class {
  ...
  getFeeds() {
    ...
    return feedFolder.children.filter((it) => {
      if (!(it instanceof import_obsidian7.TFile) || it.extension !== "md") {
        return false;
      }
      return true;
    }).map((f) => f);

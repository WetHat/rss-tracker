<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [rss-tracker](./rss-tracker.md) &gt; [DataViewJSTools](./rss-tracker.dataviewjstools.md) &gt; [rssReadingListByFeed](./rss-tracker.dataviewjstools.rssreadinglistbyfeed.md)

## DataViewJSTools.rssReadingListByFeed() method

Display collapsible reading tasks grouped by feed.

**Signature:**

```typescript
rssReadingListByFeed(items: TPageRecords, read?: boolean, expand?: boolean): Promise<void>;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

items


</td><td>

[TPageRecords](./rss-tracker.tpagerecords.md)


</td><td>


</td></tr>
<tr><td>

read


</td><td>

boolean


</td><td>

_(Optional)_ `false` to collect and display unchecked (unread) reading tasks: `true` otherwise.


</td></tr>
<tr><td>

expand


</td><td>

boolean


</td><td>

_(Optional)_ `undefined` render immediately using a generic dataview table; `true` render table immediately and expand the table by default; `false` to collapse the table by default and render the table on-demand.


</td></tr>
</tbody></table>
**Returns:**

Promise&lt;void&gt;

